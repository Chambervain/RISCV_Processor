...
class InsMem(object):
    ...
    def readInstr(self):
        with open(os.path.join(ioDir, "imem.txt")) as im:
            self.IMem = [data.replace("\n", "") for data in im.readlines()]
        # read instruction memory, return 32 bit hex val
        lenInstruction = int(len(self.IMem) / 4)
        initialInstructions = [0] * lenInstruction
        for i in range(lenInstruction):
            initialInstructions[i] = self.IMem[4 * i] + self.IMem[4 * i + 1] + self.IMem[4 * i + 2] + self.IMem[4 * i + 3]
...
class DataMem(object):
    ...
    def readInstr(self):
        # read data memory, return 32 bit hex val
        lenDataMemory = int(len(self.DMem) / 4)
        initialDataMemory = [0] * lenDataMemory
        for i in range(lenDataMemory):
            initialDataMemory[i] = self.DMem[4 * i] + self.DMem[4 * i + 1] + self.DMem[4 * i + 2] + self.DMem[4 * i + 3]
    ...
    def outputDataMem(self):
        ...
        resPath = resPath.replace("input", "output_bz2306")
...
class SingleStageCore(Core):
    ...
    def __init__(self, ioDir, imem, dmem):
        ioDir = ioDir.replace("input", "output_bz2306")
        super(SingleStageCore, self).__init__(ioDir + "/SS_", imem, dmem)
        self.opFilePath = os.path.join(ioDir, "StateResult_SS.txt")
        self.opFilePath2 = os.path.join(ioDir, "PerformanceMetrics.txt")
    ...
    def step(self):
        ...
        with open(self.opFilePath2, "w") as wf:
            printstate = ["-------------------- Single Stage Performance Metrics --------------------" + "\n"]
...
class FiveStageCore(Core):
    ...
    def step(self):
        ...
        with open(self.opFilePath2, "a") as wf:
            printstate = ["--------------------- Five Stage Performance Metrics ---------------------" + "\n"]
            printstate.append("Number of cycles taken: " + str(totalExecutionCycleSS) + "\n")
            printstate.append("Cycles per instruction: " + str(averageCPISS) + "\n")
            printstate.append("Instructions per cycle: " + str(instructionsPerCycleSS) + "\n")
            printstate.append("\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='RISCV Processor')
